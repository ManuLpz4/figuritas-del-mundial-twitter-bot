...
stream.on(ETwitterStreamEvent.Data, async (tweet) => {
  if (!tweet.data.text.includes("epes:") || !tweet.data.text.includes("olas:"))
    return;
  console.log(tweet.data.text);

  const tweetAuthor = (
    await twitterClient.v2.user(tweet.data.author_id as string)
  ).data;
  ...
  users.forEach((user: any) => {
    if (user.id !== tweetAuthor.id) {
      const repesMatches = myRepes?.filter((repe: string) =>
        user.nolas.includes(repe)
      );
      if (repesMatches?.length) {
        const nolasMatches = myNolas?.filter((nola: string) =>
          user.repes.includes(nola)
        );
        if (nolasMatches?.length) {
  ...
  repesMatchMap = new Map(
    [...repesMatchMap.entries()].sort((a, b) => b[1].length - a[1].length)
  );
  ...
  for (const repesMatch of repesMatchMap) {
    ...
    const matchReplyLine = `\nCambiale tu${
      repesMatch[1].length > 1 ? "s" : ""
    } ${repesMatch[1].toString().replace(",", ", ")} por su${
      nolasMatch.length > 1 ? "s" : ""
    } ${nolasMatch.toString().replace(",", ", ")} a @${
      matchCollector.username
    }`;
